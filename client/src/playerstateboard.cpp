...
void PlayerStateBoard::drawDefault(Widget::ROIMap m) const
{
    ...
    LabelBoard{{.label = str_printf(u8"攻击 %d - %d", combatNode. dc[0], combatNode. dc[1]).c_str(), .font{5, 15}}}.draw({.x{m.x +  21}, .y{m.y + 317}});
    LabelBoard{{.label = str_printf(u8"防御 %d - %d", combatNode. ac[0], combatNode. ac[1]).c_str(), .font{5, 15}}}.draw({.x{m.x + 130}, .y{m.y + 317}});
    LabelBoard{{.label = str_printf(u8"魔法 %d - %d", combatNode. mc[0], combatNode. mc[1]).c_str(), .font{5, 15}}}.draw({.x{m.x +  21}, .y{m.y + 345}});
    LabelBoard{{.label = str_printf(u8"魔防 %d - %d", combatNode.mac[0], combatNode.mac[1]).c_str(), .font{5, 15}}}.draw({.x{m.x + 130}, .y{m.y + 345}});
    LabelBoard{{.label = str_printf(u8"道术 %d - %d", combatNode. sc[0], combatNode. sc[1]).c_str(), .font{5, 15}}}.draw({.x{m.x + 233}, .y{m.y + 345}});

    LabelBoard{{.label = u8"攻击元素", .font{5, 15}}}.draw({.x{m.x + 10}, .y{m.y + 376}});
    LabelBoard{{.label = u8"防御元素", .font{5, 15}}}.draw({.x{m.x + 10}, .y{m.y + 406}});
    LabelBoard{{.label = u8"弱点元素", .font{5, 15}}}.draw({.x{m.x + 10}, .y{m.y + 436}});
