...
std::u8string SDItem::getXMLLayout(const std::unordered_map<int, std::string> & params) const
{
    ...
    if(ir.equip.duration > 0){
        ...
        xmlStr += str_printf(u8R"###( <par>【持久】<t color='%s'>/)###" MIR2_STR_FORMAT_SIZE_T u8R"###(/%d</t></par> )###""\n", duraColorStr, duration[0], duration[1], ir.equip.duration);
    }
...
std::unordered_set<uint64_t> SDInventory::getItemSeqIDSet() const
{
    ...
    for(const auto &item: m_list){
        if(!result.insert(buildItemSeqID(item.itemID, item.seqID)).second){
            throw fflerror("found duplicated item: itemID = " MIR2_STR_FORMAT_SIZE_T ", seqID = " MIR2_STR_FORMAT_SIZE_T, to_llu(item.itemID), to_llu(item.seqID));
        }
    }
    return result;
}
...
const SDItem &SDInventory::add(SDItem newItem, bool keepSeqID)
{
    ...
    if(keepSeqID){
        if(itemSeqIDSet.count(buildItemSeqID(newItem.itemID, newItem.seqID))){
            throw fflerror("found duplication with given item: itemID = " MIR2_STR_FORMAT_SIZE_T ", seqID = " MIR2_STR_FORMAT_SIZE_T, to_llu(newItem.itemID), to_llu(newItem.seqID));
        }
...
void SDInventory::merge(uint32_t itemID, uint32_t fromSeqID, uint32_t toSeqID)
{
    if(!(fromSeqID && toSeqID)){
        throw fflerror("invalid fromSeqID = " MIR2_STR_FORMAT_SIZE_T ", toSeqID = " MIR2_STR_FORMAT_SIZE_T, to_llu(fromSeqID), to_llu(toSeqID));
    }
    ...
    if(!ir.packable()){
        throw fflerror("item is not packable: itemID = " MIR2_STR_FORMAT_SIZE_T, to_llu(itemID));
    }
