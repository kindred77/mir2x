        ...
        case AM_SPACEMOVEOK:
            {
                ...
                if(!removeGridUID(fromUID, amTSM.X, amTSM.Y)){
                    throw fflerror("CO location error: (UID = " MIR2_STR_FORMAT_SIZE_T ", X = %d, Y = %d)", to_llu(fromUID), amTSM.X, amTSM.Y);
                }
        ...
        case AM_JUMPOK:
            {
                ...
                if(!hasGridUID(fromUID, amTJ.X, amTJ.Y)){
                    throw fflerror("CO location error: (UID = " MIR2_STR_FORMAT_SIZE_T ", X = %d, Y = %d)", to_llu(fromUID), amTJ.X, amTJ.Y);
                }
        ...
        case AM_MOVEOK:
            {
                ...
                if(!removeGridUID(amTM.UID, amTM.X, amTM.Y)){
                    throw fflerror("CO location error: (UID = " MIR2_STR_FORMAT_SIZE_T ", X = %d, Y = %d)", to_llu(amTM.UID), amTM.X, amTM.Y);
                }
...
corof::awaitable<> ServerMap::on_AM_TRYLEAVE(const ActorMsgPack &mpk)
{
    ...
    if(!removeGridUID(fromUID, amTL.X, amTL.Y)){
        throw fflerror("CO location error: (UID = " MIR2_STR_FORMAT_SIZE_T ", X = %d, Y = %d)", to_llu(fromUID), amTL.X, amTL.Y);
    }
